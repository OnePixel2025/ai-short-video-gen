        ...
        <div>
            {(!loading) && videoList?.length == 0 ?
                ...
                justify-center mt-20 gap-5 p-5 border border-dashed rounded-xl py-16'>
                    ...
                    <h2 className='text-gray-400 text-lg'>{getTranslation('youDontHaveAnyVideoCreated', language)}</h2>
                    <Link href={'/create-new-video'}>
                        <Button>{getTranslation('createNewVideoButton', language)}</Button>
                    </Link>
                </div>
                :
                    ...
                    {videoList?.length > 0 ? videoList?.map((video, index) => (
                        <Link key={index} href={'/play-video/' + video?._id}>
                            <div className='relative'>
                                ...
                                aspect-[2/3]'
                                /> :
                                    <div className='aspect-[2/3] p-5 w-full rounded-xl bg-[#18181B]
                                flex items-center justify-center gap-2'>
                                        <RefreshCcw className='animate-spin' />
                                        <h2 className='font-semibold'>{getTranslation('generating', language)}</h2>
                                    </div>}
                                <div className='absolute bottom-3 px-5 w-full'>
                                    <h2 className='font-semibold text-shadow-lg text-white'>{video?.title}</h2>
                                    <h2 className='text-sm font-semibold text-shadow-lg text-white'>{moment(video?._creationTime).fromNow()}</h2>
                                </div>
                            </div>
                        </Link>
                    )) :
